...
app.post("/insert", (req, res) => {
    ...
    console.log(PatientSoundDisorderDetails);
    // db.query("select max(patient_id) from spiel.patient_details;", (err, patient_id) => {
    //     if (err) {
    //       console.log(err);
    //     } else {
    //       res.send(result);
...
//     const sqlInsert="insert into spiel.patient_details(patient_fName,patient_mName,patient_lName,patient_gender,patient_disorder,patient_dob,parimary_complain,Caretaker_fname,Caretaker_mname,Caretaker_lname,realationship,email_id,contact_number1) values (?,?,?,?,?,?,?,?,?,?,?,?,?);"
//      db.query(sqlInsert,[first_name,middle_name,last_name,Gender,disorder,birthday,Primary_Complaint,user_name,middleName,LastName,RelationShip,email,contact_no],(err,result)=>{
//          res.send(err); 
// });
//     const sqlInsert1="insert into spiel.patient_voice_disorder_details(disorder,vocal_abuse,vocal_misue,prof_voice_user_level,medical_etiology,neurogenic_disorder,respiratory_disorders,voice_usage,task,grabs,pitch,loudness,quality,resonance,articulation,prosody,instrumental_analysis,instrumental_analysis1,multi_Dimensional_Voice_Profile,praat,aerodynamic,diagnosis_terms)values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
//     db.query(sqlInsert1,[disorder,vocal_abuse,vocal_misue,prof_voice_user_level,medical_etiology,neurogenic_disorder,respiratory_disorders,voice_usage,task,grabs,pitch,loudness,quality,resonance,articulation,prosody,instrumental_analysis,instrumental_analysis1,multi_Dimensional_Voice_Profile,praat,aerodynamic,diagnosis_terms],(err,result)=>{
//         res.send(err); 
// });
...
console.log("call spiel.create_account");
